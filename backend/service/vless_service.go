...
func NewVlessService(cfg *config.Config) *VlessService {
	return &VlessService{cfg: cfg}
}

func (s *VlessService) AddUser(ctx context.Context, username, password string) error {
	if err := s.addToMemory(ctx, username, password); err != nil {
		return fmt.Errorf("gagal add user Vless ke memory: %w", err)
	}
...
func (s *VlessService) DelUser(ctx context.Context, username string) error {
...
func (s *VlessService) addToMemory(ctx context.Context, username, password string) error {
	confPath := s.cfg.XrayVlessConfPath
	if confPath == "" {
		return fmt.Errorf("path file konfigurasi Vless belum diatur di config")
	}
	...
	tmpFile, err := os.CreateTemp("", "xray-vless-adu-*.json")
	if err != nil {
...
func (s *VlessService) addToJSON(username, password string) error {
	confPath := s.cfg.XrayVlessConfPath
...
func (s *VlessService) delFromMemory(ctx context.Context, username string) error {
	confPath := s.cfg.XrayVlessConfPath
	if confPath == "" {
		return fmt.Errorf("path file konfigurasi Vless belum diatur di config")
	}
	...
	tmpFile, err := os.CreateTemp("", "xray-vless-rmu-*.json")
	if err != nil {
		return fmt.Errorf("gagal membuat file temp: %w", err)
	}
	tmpPath := tmpFile.Name()
...
func (s *VlessService) delFromJSON(username string) error {
	confPath := s.cfg.XrayVlessConfPath
...
func (s *VlessService) atomicWriteJSON(filePath string, root map[string]interface{}) error {
...
func GenerateVlessLink(remark, hostname string, port int, uuid, net, path string, isTLS bool) string {
	security, sni := "none", ""
	if isTLS {
		security = "tls"
	...
	switch net {
	case "grpc":
		query = fmt.Sprintf("mode=gun&security=%s&encryption=none&type=grpc&serviceName=%s%s", security, path, sni)
	case "ws", "httpupgrade":
		query = fmt.Sprintf("path=%s&security=%s&encryption=none&host=%s&type=%s%s", path, security, hostname, net, sni)
	}

	return fmt.Sprintf("vless://%s@%s:%d?%s#%s", uuid, hostname, port, query, url.QueryEscape(remark))
}
